...
<html lang="en">
                ...
                <div class="manage-job-container">
                  <table class="table">
                    ...
                    <c:forEach items="${pageInfo.list }" var="job">
                      <tr class="job-items">
                        <td class="title">
                          <h5><a href="jobdetails.action?jobid=${job.jobid }">${job.jobname }</a></h5>
                          <div class="info">
                            <span class="office-location"><i data-feather="map-pin"></i>${job.jobaddr }</span>
                          </div>
                        </td>
                        <!-- 就是查询这个工作所有的申请人的列表 -->
                        <td class="application"><a href="queryJobApplyUsers.action?jobid=${job.jobid }">${job.appusernum } Application(s)</a></td>
                        <td class="deadline"><fmt:formatDate value="${job.jobdeadlinetime }" type="date"/></td>
                        <c:if test="${job.jobstate==0 }">
                        <td class="status active">Invalid</td>
                        </c:if>
                        <c:if test="${job.jobstate==1 }">
                        <td class="status active">Active</td>
                        </c:if>
                        <c:if test="${job.jobstate==2 }">
                        <td class="status active">Pending</td>
                        </c:if>
                        <td class="action">
                          <a href="jobdetails.action?jobid=${job.jobid }" class="preview"><i data-feather="eye"></i></a>
                          <!-- <a href="#" class="edit"><i data-feather="edit"></i></a> -->
                          <a href="javascript:deleteJob(${job.jobid })" class="remove"><i data-feather="trash-2"></i></a>
                        </td>
                      </tr>
                      </c:forEach>
                    </tbody>
  ...
  function deleteJob(jobid) {
    ...
    //几个参数需要注意一下
        ...
        url: "deleteJob.action?jobid="+jobid,//url
