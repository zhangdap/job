              ...
              <div class="dashboard-content-wrapper">
                <div class="manage-candidate-container">
                  <table class="table">
                    ...
                    <tbody>
                    <c:forEach items="${pageInfo.list }" var="userVo">
                      <tr class="candidates-list">
                        <td class="title">
                          <div class="thumb">
                            <img src="${userVo.userhead }" class="img-fluid" alt="">
                          </div>
                          <div class="body">
                            <h5><a href="queryResumeByUserid.action?jspName=employer-look-resume&userid=${userVo.userid }">${userVo.username }</a></h5>
                            <div class="info">
                              <span class="designation"><i data-feather="check-square"></i>${userVo.useraddr }</span>
                            </div>
                          </div>
                        </td>
                        <td class="status"><i data-feather="check-circle"></i>${userVo.jobname }</td>
                        <c:if test="${userVo.applystate==1 }">
                        <td class="status"><i data-feather="check-circle"></i>Passed</td>
                        </c:if>
                        <c:if test="${userVo.applystate==0}">
                        <td class="status"><i data-feather="check-circle"></i>Undetermined</td>
                        </c:if>
                        <td class="action">
                          <a href="${userVo.resumeurl }" class="download"><i data-feather="download"></i></a>
                          <a href="javascript:void(0)"  onclick="showDiv(${userVo.userid },${userVo.jobid })" class="inbox"><i data-feather="mail"></i></a>
                          <a href="javascript:deleteApplyUser(${userVo.userid },${userVo.jobid })" class="remove"><i data-feather="trash-2"></i></a>
                        </td>
                      </tr>
                    </c:forEach>                         
                    </tbody>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <h5 class="modal-title" id="myModalLabel">发送面试信息</h5>
            </div>
            <div class="modal-body">
            <form id="form1">
            <div>
            <font>公司名称</font>
            <input type="text" style="width: 470px" name="companyName">
            </div>
            <font>消息内容</font>
             <textarea  style="width: 470px" rows="" cols="" name="content"></textarea>
             <input type="text" hidden="hidden" id="inuserid" name="userid">
             <input type="text" hidden="hidden" id="injobid" name="jobid">
            </form>
            </div>
            <div class="modal-footer">
             <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
             <button type="button" class="btn btn-primary" onclick="subFun()">确认发送</button>
            </div>
        </div>
    </div>
   </div>
  ...
  function subFun() {
    ...
    //几个参数需要注意一下
        ...
        url: "postinfoToApply.action" ,//url
  ...
  function deleteApplyUser(userid,jobid) {
	  $.ajax({
    //几个参数需要注意一下
        type: "POST",//方法类型
        dataType: "json",//预期服务器返回的数据类型
        url: "deleteApplyUser.action?userid="+userid+"&jobid="+jobid,//url
        success: function (result) {
         if (result.info=="ok") {
        	 window.location.reload();
         }
